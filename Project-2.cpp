...
void glutSolidCube ( GLdouble size ); // size = 2.0
...
static void display(void)
{
    ...
    glColor3f(0.0,0.0,1.0); //fill color of sphere as blue
    ...
    glPushMatrix ();//place us on the position of the stack we want to be at
    glColor3f(0.0,1.0,0.0);// fill color of cube as green
    glRotatef(viewangleCz, 0,1,0);//rotates the object
    glTranslated(4.0,0.0,scaleCz);//moves the object along the z axis
    glutSolidCube ( 2.0 );//place the object
    glPopMatrix ();

    glPushMatrix ();//place us on the position of the stack we want to be at
    glColor3f(1.0,0.0,0.0);// fill color of teapot as red
...
static void key(unsigned char key, int x, int y)
{
    switch (key)
    {
        ...
        case 'r': //resets the flags so now we can zoom or rotate all objects at once.
            ...
            WireFrame = false;
            break;
        case 'w': //press w to get the wire frame of all the objects and rotate all objects together.
            flagC=true;
            flagS=true;
            flagT=true;
            WireFrame = true;
